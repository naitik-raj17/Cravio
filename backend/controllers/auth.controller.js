...
export const signUp = async(req,res)=>{
    try{
        ...
        if(mobile.length<10){
            return res.status(400).json({message:"mobile must be at least 10 characters"})
        }
...
export const verfiyOtp = async(req,res)=>{
    try{
        ...
        await user.save()
        return res.status(200).json({message:"otp verify success"})
    }
    catch(error){
...
export const googleAuth = async(req,res) =>{
    try{ 
        ...
        if(!user){
            user = await User.create({
                fullName,email,mobile,role,authProvider: "google",
            })
        }
